function Out=wtc_plotcustalex(x,y,varargin)
%% Wavelet coherence
%
% USAGE: [Rsq,period,scale,coi,sig95,Wxy]=wtc(x,y,[,settings])
%
% 
...
%         HCB=safecolorbar;%#ok
        ...
        set(gca,'YLim',log2([min(period),max(period)]), ...
            ...
            'YTickLabel',num2str(Yticks'), ...
            'layer','top', ... %%%%alex )
            'FontAngle','italic','FontName','Calibri'); %%%%alex )
        ...
        phs_dt=round(length(t)/Args.ArrowDensity(2)); tidx=max(floor(phs_dt/2),1):phs_dt:length(t);
        phs_dp=round(length(period)/Args.ArrowDensity(2)); pidx=max(floor(phs_dp/2),1):phs_dp:length(period);
        phaseplot(t(tidx),log2(period(pidx)),aaa(pidx,tidx),Args.ArrowSize,Args.ArrowHeadSize);
...
%         fill(tt, log2([period([end 1]) coi period([1 end])]),'w', 'alphadatamapping','direct','facealpha',.5);
        line(tt, log2([period([end 1]) coi period([1 end])]),'color','k','linewidth',1);
        colormap(jet);
        hold off
    end
end

Out.RR = Rsq; % R2 coherence
Out.period = period; % Periods
Out.scale = scale; % scales
Out.coi = coi; % Cone of influence
Out.wtcsig = wtcsig; % R2 significancy
Out.Wxy = Wxy; % Cross wavelets
